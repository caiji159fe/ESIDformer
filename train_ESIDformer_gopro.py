...
class Trainer:
    ...
    def train(self):
        self._init_params()
        start_epoch = 0
        if os.path.exists('last_ESIDformer_gopro.pth'):
            print('load_pretrained')
            training_state = (torch.load('last_ESIDformer_gopro.pth'))
            ...
            training_state = (torch.load('best_ESIDformer_pretrained.pth'))
            new_weight = self.netG.state_dict()
...
if __name__ == '__main__':
    with open('config/config_ESIDformer_gopro.yaml', 'r') as f:
    ...
    get_dataloader = partial(DataLoader, batch_size=batch_size, num_workers=0, shuffle=True, drop_last=False)
